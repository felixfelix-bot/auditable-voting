...
- Optional **windowed publication** (`publicationMode: "windowed"`) to reduce timing correlation between eligibility and a public ballot. The client holds each finished ballot locally, releases all held ballots in one slot at `closeAt`, stamps the shared release timestamp on both the signed event and the payload, suppresses provisional events for the round, and accepts late releases until `closeAt + finalizationGraceSeconds`. Coordinator and delegated audit proxy hold the close and final summary until that grace elapses. See [docs/time-window-publication-plan.md](docs/time-window-publication-plan.md).
...
When an issuer proxy is active, voters send blind ballot requests only to that proxy. The voter and proxy send separate authenticated, privacy-limited status messages to the organiser for live requested, issued, and received progress; the blinded request itself is not copied to the organiser, and the shipped participant status carries no identity-linked submitted signal or submission id. Voter receipt acknowledgements return to the issuing proxy, which retains the exact issuance for reliable replay. Signed public delegations include the proxy's private-DM relay set so fresh voters use relays the proxy actually reads. The organiser publishes this public routing delegation only after the initial private proxy configuration has reached a relay, so a general invite cannot route a voter to an unconfigured proxy. Blind request, issuance, and recovery traffic uses that configured relay set directly and does not consult NIP-65 relay hints.
